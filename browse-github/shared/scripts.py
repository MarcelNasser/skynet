...
def _auth():
    ...
    """
    token = os.environ.get("GITHUB_TOKEN", "xxxx")
    username = os.environ.get("GITHUB_USERNAME", "xxxx")
    if token == "xxxx":
        logger.debug('GITHUB_TOKEN uses a dummy value')
    if username == "xxxx":
        logger.debug('GITHUB_USERNAME uses a dummy value')
