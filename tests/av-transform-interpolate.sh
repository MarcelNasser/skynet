...
echo "#2 'interpolate' single audio (method=palindromic)"
bash "$ROOT_DIR"/transform-av/interpolate/run -s "$ROOT_DIR"/tests/data/audio/single-file/ -m "palindromic" -l 2  || error
[ "$(find "$ROOT_DIR/tests/data/audio/single-file/.interpolated" -maxdepth 1 -iname '*.x.wav'| wc -l)" -ne 2 ] && echo "number of interpolates <> 2" && error

rm -r "$ROOT_DIR/tests/data/audio/single-file/.chopped/"
rm -r "$ROOT_DIR/tests/data/audio/single-file/.interpolated/"
