...
bash "$ROOT_DIR"/transform-av/compute/run -s "$ROOT_DIR"/tests/data/audio/single-file -m expensive -l 1 >/dev/null || error
...
[ "$(find "$ROOT_DIR/tests/data/audio/single-file/.fft" -maxdepth 1 -iname '*.png'| wc -l)" -ne 2 ] && echo "number of fft [expensive] <> 2" && error

echo "#1 'compute-audio-space' single audio [fractal]"
bash "$ROOT_DIR"/transform-av/compute/run -s "$ROOT_DIR"/tests/data/audio/single-file -m fractal -l 1 >/dev/null || error
check_content "$ROOT_DIR"/tests/data/audio/single-file/.fft 'image/png'
...
bash "$ROOT_DIR"/transform-av/compute/run -s "$ROOT_DIR"/tests/data/audio/multiple-files -m reverse >/dev/null || error
